...
class EditPageAboutController extends Controller
{
    ...
    public function updateEncabezado(Request $request)
    {
        ...
        $existe = DB::table('nosotros_encabezado')->where('id_nosotros', $idNosotros)->exists();
        $data = ['titulo' => $request->titulo, 'subtitulo' => $request->subtitulo, 'updated_at' => now()];

        if ($existe) {
            DB::table('nosotros_encabezado')->where('id_nosotros', $idNosotros)->update($data);
        } else {
            DB::table('nosotros_encabezado')->insert(array_merge($data, ['id_nosotros' => $idNosotros, 'created_at' => now()]));
        }

        return response()->json(['success' => true, 'message' => 'Encabezado guardado correctamente.']);
    }
    ...
    public function updateHistoria(Request $request)
    {
        ...
        $idNosotros = $this->getOrCreateNosotros();
        $historiaExistente = DB::table('nosotros_historia')->where('id_nosotros', $idNosotros)->first();
        ...
        if ($request->hasFile('imagen_historia')) {
            if ($historiaExistente?->imagen) Storage::disk('public')->delete($historiaExistente->imagen);
            $data['imagen'] = $request->file('imagen_historia')->store('nosotros', 'public');
        }
        ...
        if ($historiaExistente) {
            DB::table('nosotros_historia')->where('id_nosotros', $idNosotros)->update($data);
        } else {
            DB::table('nosotros_historia')->insert(array_merge($data, ['id_nosotros' => $idNosotros, 'created_at' => now()]));
        }

        return response()->json(['success' => true, 'message' => 'Historia guardada correctamente.']);
    }
    ...
    public function updateIdentidad(Request $request)
    {
        ...
        $idNosotros = $this->getOrCreateNosotros();
        $identidadExistente = DB::table('nosotros_identidad')->where('id_nosotros', $idNosotros)->first();
        $dataIdentidad = ['titulo' => $request->identidad_titulo, 'subtitulo' => $request->identidad_subtitulo, 'updated_at' => now()];

        if ($identidadExistente) {
            DB::table('nosotros_identidad')->where('id_nosotros', $idNosotros)->update($dataIdentidad);
            $idIdentidad = $identidadExistente->id_identidad;
        } else {
            $idIdentidad = DB::table('nosotros_identidad')->insertGetId(array_merge($dataIdentidad, ['id_nosotros' => $idNosotros, 'created_at' => now()]));
        }
        ...
        foreach ($items as $tipo => $item) {
            $existeItem = DB::table('nosotros_identidad_items')->where('id_identidad', $idIdentidad)->where('tipo', $tipo)->exists();
            $dataItem = ['titulo' => $item['titulo'], 'contenido' => $item['contenido'], 'orden' => $item['orden'], 'updated_at' => now()];

            if ($existeItem) {
                DB::table('nosotros_identidad_items')->where('id_identidad', $idIdentidad)->where('tipo', $tipo)->update($dataItem);
            } else {
                DB::table('nosotros_identidad_items')->insert(array_merge($dataItem, ['id_identidad' => $idIdentidad, 'tipo' => $tipo, 'created_at' => now()]));
            }
        }

        return response()->json(['success' => true, 'message' => 'Identidad guardada correctamente.']);
    }

    private function getOrCreateNosotros(): int
    {
        $nosotros = DB::table('nosotros')->where('id_pagina', self::ID_PAGINA)->first();
        if ($nosotros) return $nosotros->id_nosotros;
        return DB::table('nosotros')->insertGetId(['id_pagina' => self::ID_PAGINA, 'created_at' => now(), 'updated_at' => now()]);
    }
}
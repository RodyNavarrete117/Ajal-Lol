...
<body>
    ...
    {{-- ↓ ÚNICO CAMBIO: session() en lugar de auth() + nombres de tabla en español --}}
    @if(session('user_id'))
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const initialCount = {{ 
                \App\Models\Notificacion::where('id_usuario', session('user_id'))
                    ->where('leido', 0)
                    ->count() 
            }};
            if (typeof updateNotificationCount === 'function') {
                updateNotificationCount(initialCount);
            }
        });
    </script>
    @endif
